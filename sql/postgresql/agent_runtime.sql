...
-- 跨领域协作不复用 operation：一个 operation 只能绑定一个领域动作和一个
-- 审批/幂等边界。协调批次只保存 DAG、步骤回执与恢复状态，步骤实际写入仍由
-- 各领域自己的 Operation/Effect 完成。
CREATE TABLE IF NOT EXISTS agent_runtime.coordination_batch (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT ck_agent_runtime_coordination_batch_status CHECK (
        status IN ('CREATED', 'RUNNING', 'WAITING_APPROVAL', 'PARTIAL_SUCCEEDED',
                   'SUCCEEDED', 'FAILED', 'CANCELLED')
    ),
    CONSTRAINT ck_agent_runtime_coordination_batch_version CHECK (version >= 1)
);

CREATE INDEX IF NOT EXISTS idx_agent_runtime_coordination_batch_scope
    ON agent_runtime.coordination_batch (tenant_id, user_id, thread_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_agent_runtime_coordination_batch_status
    ON agent_runtime.coordination_batch (status, updated_at DESC);

CREATE TABLE IF NOT EXISTS agent_runtime.coordination_step (
    batch_id VARCHAR(128) NOT NULL REFERENCES agent_runtime.coordination_batch(batch_id),
    step_id VARCHAR(128) NOT NULL,
    domain VARCHAR(64) NOT NULL,
    action_id VARCHAR(128) NOT NULL,
    executor_tool VARCHAR(128) NOT NULL,
    work_order JSONB NOT NULL,
    depends_on JSONB NOT NULL DEFAULT '[]'::jsonb,
    failure_policy VARCHAR(32) NOT NULL DEFAULT 'BLOCK_DEPENDENTS',
    ...
    CONSTRAINT ck_agent_runtime_coordination_step_status CHECK (
        status IN ('PENDING', 'RUNNING', 'WAITING_APPROVAL', 'SUCCEEDED',
                   'FAILED', 'SKIPPED', 'CANCELLED')
    ),
    CONSTRAINT ck_agent_runtime_coordination_step_failure_policy CHECK (
        failure_policy IN ('CONTINUE', 'BLOCK_DEPENDENTS')
    ),
    CONSTRAINT ck_agent_runtime_coordination_step_version CHECK (version >= 1)
);

ALTER TABLE agent_runtime.coordination_step
    ADD COLUMN IF NOT EXISTS version BIGINT NOT NULL DEFAULT 1;

CREATE INDEX IF NOT EXISTS idx_agent_runtime_coordination_step_dispatch
    ON agent_runtime.coordination_step (status, batch_id, step_id);
CREATE INDEX IF NOT EXISTS idx_agent_runtime_coordination_step_operation
    ON agent_runtime.coordination_step (operation_id)
    WHERE operation_id IS NOT NULL;

INSERT INTO agent_runtime.schema_migration(version)
VALUES ('agent_runtime_coordination_v3')
ON CONFLICT (version) DO NOTHING;
